...
# Data Exploration of eDNA data for RIVPACS model-building
...
The following excel files were used for this data: 21072022_eDNA genera.csv, 21072022_eDNA sample metadata.csv, habitat_lv12_RIVPACS.csv, eDNA_NCC_WatershedStress.csv, and eDNA_EPT_taxa.csv

Sites were aligned from the two .csv files and "SJR-" sites (Wolastoq sites) were removed as these are large river sites. 
...
```
Taxa richness appears normally distributed & without large differences between test and reference sites. However the species that make up this richness could differ between sites. This is explored in more detail below. 
...
The most and least common species were gathered from the distribution above by looking at the species that fell within the 5th and 95th quantiles. The taxa in the following graphs are the most and least common species, for reference and test sites.
...
Next, the percent of EPT taxa for test and reference sites was assessed, to begin to check to see if there are any differences in species communities. EPT taxa belonging to the orders Ephemeroptera, Plecoptera, and Trichoptera were chosen because these are often useful bioindicator taxa. These were further broken down by looking specifically at the percent of taxa that belong to the Plecoptera order, as this order contains more sensitive species, which might indicate differences between communities at test and reference sites. 
...
Watershed stressor scores were obtained using ArcGIS. These scores represent land use or stressors within the watershed that could drive ecological change, such as road density, agriculture, foresty or other industrial use. 
...
#there is a difference between the means for percent ept, for status
...
Watershed Stressor Scores were divided into six ranges, and the total taxa abundance was summed across each range of watershed stressor scores. The following tile plots show the total taxa abundance for each species, across all test and reference sites. 
...
## Distribution of Environmental Variables for test and reference sites
The distribution of each environmental variable was graphed in reference and test sites to see if there are differences in environmental variables. These were also graphed by latitude and longitude to assess spatial variation.
